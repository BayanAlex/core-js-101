...
function getPolynom(...coeffs) {
  ...
  return function result(arg) {
    return coeffs.reduce((res, coeff, i) => res + coeff * (arg ** (coeffs.length - i - 1)), 0);
  };
}
...
function retry(func, attempts) {
  ...
  return function retryFunc(...args) {
    try {
      ...
      count += 1;
      if (count < attempts) return retryFunc(...args);
    }
    return undefined;
  };
}
...
function logger(func, logFunc) {
  return function log(...args) {
    const msg = `${func.name}(${JSON.stringify(args).slice(1, -1)})`;
    logFunc(`${msg} starts`);
